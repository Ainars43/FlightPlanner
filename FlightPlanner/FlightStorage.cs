...
namespace FlightPlanner
{
    public class FlightStorage : IFlightStorage
    {
        ...
        {
            return _context.Flights.Where(f => f.From.AirportCode == request.From && f.To.AirportCode == request.To && DateTime.Parse((string)f.DepartureTime).Date == DateTime.Parse(request.DepartureDate)).ToList();
        }
    }
}
